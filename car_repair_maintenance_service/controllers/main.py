...
class CarRepairSupport(http.Controller):
    ...
    def request_submitted(self, **post):
        ...
        if Partner:
            ...
            #crear lista de procusto o serivicos
            list_product=post['salida_check']
            array_custom_product=[]
            for item in list_product.split(','):
                if item:
                    producto=request.env['product.product'].sudo().search([('id','=',int(item))])
                    for pd in producto:
                        array_custom_product.append(
                        (0,0,{
                        'product_id':pd.id,
                        'costo':pd.lst_price,
                        'product_uom':pd.uom_id.id,
                        'auto_checks':True,
                        })
                        )

            currency_id=request.env['car_repair.tax'].sudo().search([],limit=1).currency_id.id

            auto_cliente=post['auto_cliente']
            if not auto_cliente:
                auto_cliente=0

            stage_id=request.env['etapas.reparacion'].sudo().search([('value','=','new')],limit=1).id
...
#                                                           
                                                            ...
                                                            'website_cliente_id':Partner.id,                                                           
                                                            'website_year': post['year'],
                                                            'website_placa':post['placa'],
                                                            'website_numero_serie':post['serie'],
                                                            'custome_client_user_id': request.env.user.id,
                                                            'cosume_part_ids':array_custom_product,
                                                            'currency_id':currency_id,
                                                            'flota_cliente':auto_cliente,
                                                            'website_problema_reportado':post['description'],
                                                            'website_naturaleza_servicio':post['subject'],
                                                            'website_create':True,
                                                            'state':'new',
                                                            'stage_id':stage_id,
                                                             })
