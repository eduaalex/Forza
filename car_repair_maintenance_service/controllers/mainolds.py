...
#            'nature_of_service_id': int(post['service_id']),
    ...
    def request_submitted(self, **post):
        ...
        if Partner:
            ...
            # request.env['session_no.usuario'].sudo().search([('correo','=',request.env.user.email)]).unlink()
            return request.render('car_repair_maintenance_service.thanks_mail_send_car', values)
        else:
        #     vars={
        #     'nombre':post['customer_id'],
        #     'correo':post['email'],
        #     'telefono':post['phone'],
        #     'idservicio':post['service_id'],
        #     'marca':post['brand'],
        #     'modelo':post['model'],
        #     'ano':post['year'],
        #     'color':post['color'],
        ...
        #     'placa':post['placa']
        #     }
        #     request.env['session_no.usuario'].create(vars)

            return request.render('car_repair_maintenance_service.support_invalid_car',{'user':request.env.user,'email':post['email']})
